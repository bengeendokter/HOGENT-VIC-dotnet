...
internal class EntityConfiguration<T> : IEntityTypeConfiguration<T> where T : Entity
{
    public virtual void Configure(EntityTypeBuilder<T> builder)
    {
        ...
        builder.Property(x => x.CreatedAt).HasDefaultValueSql("NOW()").HasConversion<long>();
        builder.Property(x => x.UpdatedAt).HasDefaultValueSql("NOW()").HasConversion<long>().IsConcurrencyToken();
    }
}
